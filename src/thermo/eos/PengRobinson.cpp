 ...
 * @brief Implementation of Peng-Robinson EOS (cubic)
 */
...
PengRobinsonEOS::PengRobinsonEOS(const Core::Mixture& mixture)
    : mixture_(mixture)
...
void PengRobinsonEOS::initializeCriticalCache() {
    Tc_.assign(nc_, 0.0);
    Pc_.assign(nc_, 0.0);
    ...
    for (int i = 0; i < nc_; ++i) {
        ...
        if (!(std::isfinite(Tc) && std::isfinite(Pc) && std::isfinite(omega)) || Tc <= 0.0 || Pc <= 0.0) {
            throw std::runtime_error("Peng-Robinson EOS requires Tc/Pc/omega from databanks for component '" + comp.name() + "'");
        }
...
PengRobinsonEOS::MixParams PengRobinsonEOS::mixtureParams(double T, const std::vector<double>& x, bool compute_temperature_derivatives) const {
    ...
    for (int i = 0; i < nc_; ++i) {
        ...
        mp.b_i[i] = PR_OMEGA_B * R * Tc / Pc;
        mp.b_mix += x[i] * mp.b_i[i];
    }

    // Mixture a via quadratic mixing and cache a_i_mix = Σ_j x_j a_ij.
    for (int i = 0; i < nc_; ++i) {
        for (int j = 0; j < nc_; ++j) {
...
double PengRobinsonEOS::pressureFromMix(double T, double v, double a, double b) const {
    const double R = Constants::GAS_CONSTANT;
    if (v <= b) {
...
std::vector<double> PengRobinsonEOS::lnPhi(double T, double rho, const std::vector<double>& x, double& Z_out) const {
    ...
    if (!std::isfinite(P) || !std::isfinite(Z) || P <= 0.0) {
        throw std::runtime_error("Peng-Robinson EOS: invalid state (non-finite pressure/Z)");
    }
    ...
    if (Z <= B) {
        throw std::runtime_error("Peng-Robinson EOS: invalid state (Z <= B)");
    }

    const double term_log = std::log((Z + (1.0 + SQRT2) * B) / (Z + (1.0 - SQRT2) * B));
    const double factor = A / (2.0 * SQRT2 * B);
    ...
    for (int i = 0; i < nc_; ++i) {
        ...
        ln_phi[i] = bi_over_b * (Z - 1.0)
                  - std::log(Z - B)
                  - factor * (da_over_a - bi_over_b) * term_log;
    }
...
double PengRobinsonEOS::residualHelmholtzFromLnPhi(
    double Z,
...
EOSResult PengRobinsonEOS::calculate(
    double T,
    double rho,
    const std::vector<double>& x,
    const DerivativeSpec& deriv_spec) const
{
    ...
    try {
        ...
        if (deriv_spec.composition) {
            result.chemical_potential = result.ln_phi; // mu_res/RT = ln(phi_i)
        }
        ...
        if (deriv_spec.second_order) {
            ...
            if (!(std::isfinite(a) && std::isfinite(b) && std::isfinite(da_dT) && std::isfinite(d2a_dT2)) || b <= 0.0) {
                throw std::runtime_error("PengRobinsonEOS::calculate: invalid mixture parameters for second derivatives");
            }

            const double brho = b * rho;
            const double one_minus_brho = 1.0 - brho;
            if (!(std::isfinite(one_minus_brho) && one_minus_brho > 0.0)) {
                throw std::runtime_error("PengRobinsonEOS::calculate: invalid packing (b*rho >= 1)");
            }
            ...
            if (!(std::isfinite(num) && std::isfinite(den) && num > 0.0 && den > 0.0)) {
                throw std::runtime_error("PengRobinsonEOS::calculate: invalid log arguments for second derivatives");
            }

            const double F = std::log(num / den);
            const double dlog = cp / num - cm / den;
            const double ddlog = -(cp * cp * b) / (num * num) + (cm * cm * b) / (den * den);
            ...
            result.d2a_dT2 = -(F / (2.0 * SQRT2 * b * R)) * g2;
            result.d2a_drho2 = (b * b) / (one_minus_brho * one_minus_brho) - (a / (2.0 * SQRT2 * R * T)) * ddlog;
            result.d2a_dTdrho = -(dlog / (2.0 * SQRT2 * R)) * g1;
        }
...
double PengRobinsonEOS::pressure(double T, double rho, const std::vector<double>& x) const {
    const auto mp = mixtureParams(T, x);
    const double v = 1.0 / rho;
    return pressureFromMix(T, v, mp.a_mix, mp.b_mix);
}

std::vector<double> PengRobinsonEOS::fugacityCoefficients(double T, double rho, const std::vector<double>& x) const {
    double Z = 0.0;
...
double PengRobinsonEOS::compressibility(double T, double rho, const std::vector<double>& x) const {
    const double R = Constants::GAS_CONSTANT;
    const double v = 1.0 / rho;
...
double PengRobinsonEOS::residualHelmholtz(double T, double rho, const std::vector<double>& x) const {
    double Z = 0.0;
    const auto ln_phi = lnPhi(T, rho, x, Z);
    return residualHelmholtzFromLnPhi(Z, x, ln_phi);
}

double PengRobinsonEOS::dadt(double T, double rho, const std::vector<double>& x) const {
    if (!(std::isfinite(T) && std::isfinite(rho)) || T <= 0.0 || rho <= 0.0) {
        throw std::invalid_argument("PengRobinsonEOS::dadt: invalid T/rho");
    }
    ...
    if (!(std::isfinite(a) && std::isfinite(b) && std::isfinite(da_dT)) || b <= 0.0) {
        throw std::runtime_error("PengRobinsonEOS::dadt: invalid mixture parameters");
    }
    ...
    if (!(std::isfinite(num) && std::isfinite(den) && num > 0.0 && den > 0.0)) {
        throw std::runtime_error("PengRobinsonEOS::dadt: invalid log arguments");
    }

    const double log_term = std::log(num / den);
    const double invT = 1.0 / T;
    const double denom = 2.0 * SQRT2 * b * R;
    const double value = -(log_term / denom) * (da_dT * invT - a * invT * invT);
    if (!std::isfinite(value)) {
        throw std::runtime_error("PengRobinsonEOS::dadt: non-finite result");
    }
    return value;
}

double PengRobinsonEOS::dadrho(double T, double rho, const std::vector<double>& x) const {
    if (!(std::isfinite(T) && std::isfinite(rho)) || T <= 0.0 || rho <= 0.0) {
        throw std::invalid_argument("PengRobinsonEOS::dadrho: invalid T/rho");
    }
    ...
    if (!(std::isfinite(a) && std::isfinite(b)) || b <= 0.0) {
        throw std::runtime_error("PengRobinsonEOS::dadrho: invalid mixture parameters");
    }

    const double brho = b * rho;
    if (!(std::isfinite(brho) && brho < 1.0)) {
        throw std::runtime_error("PengRobinsonEOS::dadrho: invalid packing (b*rho >= 1)");
    }

    const double one_minus_brho = 1.0 - brho;
    if (!(std::isfinite(one_minus_brho) && one_minus_brho > 0.0)) {
        throw std::runtime_error("PengRobinsonEOS::dadrho: invalid log arguments");
    }
    ...
    if (!std::isfinite(value)) {
        throw std::runtime_error("PengRobinsonEOS::dadrho: non-finite result");
    }
    return value;
}

double PengRobinsonEOS::dPdrho(double T, double rho, const std::vector<double>& x) const {
    const auto mp = mixtureParams(T, x);
    const double v = 1.0 / rho;
    const double b = mp.b_mix;
    const double a = mp.a_mix;
...
std::vector<double> PengRobinsonEOS::compressibilityRootsTP(double T, double P, const std::vector<double>& x) const {
    if (!(std::isfinite(T) && std::isfinite(P) && T > 0.0 && P > 0.0)) {
        throw std::invalid_argument("PengRobinsonEOS::compressibilityRootsTP: invalid T/P");
    }
...
std::vector<double> PengRobinsonEOS::densityRootsTP(double T, double P, const std::vector<double>& x) const {
    const double R = Constants::GAS_CONSTANT;
...
double PengRobinsonEOS::criticalTemperature(int i) const {
    if (i < 0 || i >= nc_) throw std::out_of_range("Component index out of range");
    return Tc_[i];
}

double PengRobinsonEOS::criticalPressure(int i) const {
    if (i < 0 || i >= nc_) throw std::out_of_range("Component index out of range");
    return Pc_[i];
}

double PengRobinsonEOS::acentricFactor(int i) const {
    if (i < 0 || i >= nc_) throw std::out_of_range("Component index out of range");
    return omega_[i];
}
